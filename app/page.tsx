...
export default function Home() {
  ...
  const canSubmit =
    file !== null && apiKey.trim().length > 0 && effectiveModel.length > 0 && status !== "loading";
  ...
  async function handleSubmit() {
    ...
    try {
      // Kasıtlı olarak bir istemci zaman aşımı YOK: n8n cevap verene kadar bekliyoruz.
      ...
      <main className="mx-auto max-w-[720px] px-6 py-10">
        {status !== "success" && (
          <ol className="space-y-6">
            ...
            <StepCard n={3} title="Model seçin">
              <p className="text-sm text-ink/70">
                Lokalizasyonu hangi OpenRouter modeli yapsın? Hız/kalite dengesine göre seçin.
              </p>
              <select
                value={model}
                onChange={(e) => setModel(e.target.value)}
                className="mt-3 w-full rounded-sm border border-ink/20 bg-background px-4 py-2.5 text-sm text-ink focus:border-ink focus:outline-none"
              >
              ...
              {model === "__custom__" && (
                <input
                  ...
                  placeholder="örn. mistralai/mistral-large"
                  className="mt-2 w-full rounded-sm border border-ink/20 bg-background px-4 py-2.5 font-mono text-sm text-ink placeholder:text-ink/30 focus:border-ink focus:outline-none"
                />
              )}
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 text-xs text-teal underline underline-offset-4"
              >
                OpenRouter model listesine bak
              </a>
            </StepCard>

            <StepCard n={4} title="OpenRouter API anahtarınızı girin">
              <p className="text-sm text-ink/70">
            ...
            <StepCard n={5} title="Lokalize et">
              <button
              ...
              {status === "loading" && (
                <p className="mt-3 text-xs text-ink/60">
                  ...
                  ediliyor. <strong>Süre sınırı yok</strong> — cümle sayısına ve modele bağlı olarak
                  uzun sürebilir, n8n cevap verene kadar bekleniyor. Geçen süre:{" "}
                  <span className="font-mono">{formatElapsed(elapsedSeconds)}</span>. Sayfayı
                  kapatmayın veya sekmeden ayrılmayın.
                </p>
              )}
              {status === "error" && error && (
